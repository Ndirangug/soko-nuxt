/* eslint-disable no-console */
export const getGoogleCloudIAPAuthToken = async function () {
  // follow this tutorial
  // https://engineering.q42.nl/google-identity-aware-proxy/

  const jwt = require('jsonwebtoken')
  const axios = require('axios')

  const onehourInSeconds = 60 * 60

  const JWTProperties = {
    iss: 'soko-51dd8@appspot.gserviceaccount.com', // the client email from your serviceAccount.json    (the service account you are trying to authorize)
    aud: 'https://www.googleapis.com/oauth2/v4/token', // the google openIDTokenURL
    iat: Math.floor(new Date().getTime() / 1000), // time now in seconds
    exp: Math.floor(new Date().getTime() / 1000) + onehourInSeconds, // expiry time of the token. keep it at a maximum of 60 minutes
    target_audience:
      '1079250023096-9ecq9q30ci0nvdt9jkbkvcbl6lier7kl.apps.googleusercontent.com', // client id of the IAP secured resource
  }

  // the private key from your serviceAccount.json      (the service account you are trying to authorize)
  const privateKey =
    '-----BEGIN PRIVATE KEY-----\\n6Kp9PboHCvrzKswNk4qXbORf\n-----END PRIVATE KEY-----'

  const generatedToken = jwt.sign(JWTProperties, privateKey, {
    algorithm: 'RS256',
  })

  const openIDTokenURL = 'https://www.googleapis.com/oauth2/v4/token'
  let response = {}

  try {
    response = await axios.post(openIDTokenURL, {
      grant_type: 'urn:ietf:params:oauth:grant-type:jwt-bearer',
      assertion: generatedToken,
    })

    return response.data.id_token !== undefined ? response.data.id_token : ''
  } catch (error) {
    console.log(error)
  }
}
